...
- (void)checkRequestStatus
{
	...
	if ([self shouldTimeOut]) {			
		...
		DLog(@"Timeout Alert View");
		
		UIAlertView *a = [[UIAlertView alloc] initWithTitle:@"Could not connect to server" message:@"Unable to contact the server with this request, please try again." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
		[a performSelectorInBackground:@selector(show) withObject:nil];
		[a release];
...
+ (void)updateProgressIndicator:(id<ASIProgressDelegate> *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
	#if TARGET_OS_IPHONE
...
{	
	...
	// Find out how much data we've uploaded so far
	[self setLastBytesSent:totalBytesSent];	
	[self setTotalBytesSent:[CFBridgingRelease([(NSNumber *)CFReadStreamCopyProperty((CFReadStreamRef)[self readStream], kCFStreamPropertyHTTPRequestBytesWrittenCount) autorelease]) unsignedLongLongValue]];
	[self setComplete:YES];
	[self updateProgressIndicators];
	...
	if ([self temporaryFileDownloadPath]) {
		...
		// Decompress the file (if necessary) directly to the destination path
			...
			if (decompressionStatus != Z_OK) {
				fileError = [NSError errorWithDomain:NetworkRequestErrorDomain code:ASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Decompression of %@ failed with code %d",[self temporaryFileDownloadPath],decompressionStatus],NSLocalizedDescriptionKey,nil]];
			}
			[self removeTemporaryDownloadFile];
		} else {
...
{
	NSError *underlyingError = CFBridgingRelease([(NSError *)CFReadStreamCopyError((CFReadStreamRef)[self readStream]) autorelease]);
...
+ (NSArray *)proxiesForURL:(NSURL *)theURL fromPAC:(NSURL *)pacScriptURL
{
	...
	NSArray *proxies = CFBridgingRelease((NSArray *)CFNetworkCopyProxiesForAutoConfigurationScript((CFStringRef)script,(CFURLRef)theURL, &err2));
	if (err2) {
		return nil;
	}
	return proxies;
